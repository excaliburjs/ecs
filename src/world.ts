...
import { SystemCtor, SystemManager, isSystemConstructor } from './system-manager.ts';
import { TagQuery } from './tag-query.ts';

/**
 * The World is a self-contained entity component system for a particular context.
 */
export class World<TContext = any> {
  ...
  update(type: SystemType, elapsed: number) {
    if (type === SystemType.Update) {
      this.entityManager.updateEntities(this.context, elapsed);
    }
    this.systemManager.updateSystems(type, this.context, elapsed);
