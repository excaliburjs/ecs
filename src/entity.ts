import { Component, ComponentCtor, isComponentCtor } from './component.ts';

import { Observable, Message } from './observable.ts';
import { InitializeEvent, PreUpdateEvent, PostUpdateEvent, AddEvent, RemoveEvent, KillEvent } from './events.ts';
import { EventEmitter, EventKey, Handler, Subscription } from './event-emitter.ts';
import { MaybeKnownComponent } from './types.ts';
 ...
 * ```
 */
export class Entity<TKnownComponents extends Component = any, TContext = any> {
   ...
   * @internal
   */
  public _initialize(context: TContext) {
    if (!this.isInitialized) {
      this.onInitialize(context);
      this.events.emit('initialize', new InitializeEvent(context, this));
   ...
   * @internal
   */
  public _add(context: TContext) {
    if (!this.isAdded && this.isActive) {
      this.onAdd(context);
      this.events.emit('add', new AddEvent(context, this));
      this._isAdded = true;
    }
  }
   ...
   * @internal
   */
  public _remove(context: TContext) {
    if (this.isAdded && !this.isActive) {
      this.onRemove(context);
      this.events.emit('remove', new RemoveEvent(context, this));
      this._isAdded = false;
    }
  }
   ...
   * @internal
   */
  public _preupdate(context: TContext, elapsed: number): void {
    this.events.emit('preupdate', new PreUpdateEvent(context, elapsed, this));
    this.onPreUpdate(context, elapsed);
  }
   ...
   * @internal
   */
  public _postupdate(context: TContext, elapsed: number): void {
    this.events.emit('postupdate', new PostUpdateEvent(context, elapsed, this));
    this.onPostUpdate(context, elapsed);
  }
   ...
   */
  public onInitialize(context: TContext): void {
    // Override me
  }
   ...
   */
  public onAdd(context: TContext): void {
    // Override me
  }
   ...
   */
  public onRemove(context: TContext): void {
    // Override me
  }
   ...
   */
  public onPreUpdate(context: TContext, elapsed: number): void {
    // Override me
  }
   ...
   */
  public onPostUpdate(context: TContext, elapsed: number): void {
    // Override me
  }
   ...
   * @param elapsed
   */
  public update(context: TContext, elapsed: number): void {
    this._initialize(context);
    this._add(context);
    this._preupdate(context, elapsed);
    for (const child of this.children) {
      child.update(context, elapsed);
    }
    this._postupdate(context, elapsed);
